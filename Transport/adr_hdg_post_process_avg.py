...
Dm = 1e-5 # diffusion (m²/s)
d_l = -2*Lx/ny/refine # longitidinal dispersion (m)
...
a_c = (wh*(ch - c0)/dtc*fd.dx -
       ...
       2*fd.avg(wh*fd.dot(qhat, n))*fd.dS +
       wh*fd.dot(qhat, n)*fd.ds)
...
G_q = mu_h*fd.inner(Diff*fd.grad(ch) + velocity * ch, n)*fd.ds(outflow) + \
    ...
    mu_h*fd.Constant(cIn)*fd.ds(inlet)
...
outfile = fd.File(f"plots/adr-hdg-pp-avg-n-{refine}-p-{order}.pvd", project_output=True)
dtc.assign(dt_)
dt = dt_
